...
class parserTest extends FreeSpec with WollokParser with ParserMatchers[WollokParser] {
  ...
  "Wollok parser" - {
    ...
    "class" in {
      ...
      """class C inherits D mixed with M and N {}""" should beParsedTo (Class("C", Some("D"), ("M": FullyQualifiedReference) :: ("N": FullyQualifiedReference) :: Nil))
      """class C mixed with M and N {}""" should beParsedTo (Class("C", None, ("M": FullyQualifiedReference) :: ("N": FullyQualifiedReference) :: Nil))
    ...
    "singleton" in {
      ...
      """object O inherits D mixed with M and N {}""" should beParsedTo (Singleton("O", Some("D", Nil), ("M": FullyQualifiedReference) :: ("N": FullyQualifiedReference) :: Nil))
      """object O mixed with M and N {}""" should beParsedTo (Singleton("O", None, ("M": FullyQualifiedReference) :: ("N": FullyQualifiedReference) :: Nil))
  ...
  "literal" in {
    ...
    """true""" should beParsedTo[Literal[Boolean], Expression] (Literal(true))
    """false""" should beParsedTo[Literal[Boolean], Expression] (Literal(false))

    """null""" should beParsedTo[Literal[Null], Expression] (Literal(null))
    ...
    """object inherits D mixed with M and N {}""" should beParsedTo[Literal[Singleton], Expression] (Literal(Singleton("", Some("D", Nil), ("M": FullyQualifiedReference) :: ("N": FullyQualifiedReference) :: Nil)))
    """object mixed with M and N {}""" should beParsedTo[Literal[Singleton], Expression] (Literal(Singleton("", None, ("M": FullyQualifiedReference) :: ("N": FullyQualifiedReference) :: Nil)))
...
trait ParserMatchers[ParsersDefinition <: Parsers { type Elem = Char }] extends Matchers {

  case class beParsedTo[U <: T, T](expected: U)(implicit parser: ParsersDefinition#Parser[T]) extends Matcher[String] {
    def apply(target: String) = {
      val result = parser(new CharSequenceReader(target))

      MatchResult(
        result.successful && result.next.atEnd && result.get == expected,
        if (result.successful) if (result.next.atEnd) s"Parsed ${result.get} did not equal $expected" else s"Unconsumed input at ${result.next.pos}" else s"Parse failed! $result",
  ...
  case class beParsed[T]()(implicit parser: ParsersDefinition#Parser[T]) extends Matcher[String] {
    def apply(target: String) = {
      val result = parser(new CharSequenceReader(target))
      MatchResult(
        result.successful && result.next.atEnd,
